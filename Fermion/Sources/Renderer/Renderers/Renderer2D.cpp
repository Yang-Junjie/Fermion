...
namespace Fermion {
namespace {
std::unique_ptr<Renderer2D> s_GlobalRenderer2D;
}
...
void Renderer2D::InitGlobal(const RendererConfig& config)
{
    if (s_GlobalRenderer2D) {
        ShutdownGlobal();
    }

    s_GlobalRenderer2D = std::make_unique<Renderer2D>();
    s_GlobalRenderer2D->init(config);
}

void Renderer2D::ShutdownGlobal()
{
    if (!s_GlobalRenderer2D) {
        return;
    }

    s_GlobalRenderer2D->shutdown();
    s_GlobalRenderer2D.reset();
}

Renderer2D& Renderer2D::Get()
{
    FERMION_ASSERT(s_GlobalRenderer2D, "Renderer2D global instance is not initialized");
    return *s_GlobalRenderer2D;
}

Renderer2D* Renderer2D::TryGet()
{
    return s_GlobalRenderer2D.get();
}
...
void Renderer2D::init(const RendererConfig& config)
{
    ...
    m_PassQueue = std::make_unique<RenderPassQueue>();
...
void Renderer2D::shutdown()
{
    ...
    if (m_QuadBatch) {
        m_QuadBatch->shutdown();
    }
    if (m_CircleBatch) {
        m_CircleBatch->shutdown();
    }
    if (m_LineBatch) {
        m_LineBatch->shutdown();
    }
    if (m_TextBatch) {
        m_TextBatch->shutdown();
    }
}

void Renderer2D::updateCameraUBO(const glm::mat4& viewProj,
                                 const glm::mat4& view,
                          ...
                          int objectID)
{
    ...
    float textureIndex = m_QuadBatch->getTextureIndex(texture);
    m_QuadBatch->submit(
        transform, tintColor, QuadBatch::DefaultTexCoords, textureIndex, tilingFactor, objectID);
    m_Stats.quadCount++;
}
                                   ...
                                   int objectId)
{
    ...
    glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) * billboardRotation *
                          glm::scale(glm::mat4(1.0f), {size.x, size.y, 1.0f});
...
void Renderer2D::drawAABB(const AABB& aabb,
...
void Renderer2D::drawQuadInstanced(const glm::mat4& transform,
                                   const glm::vec4& color,
                                   const std::shared_ptr<Texture2D>& texture,
                                   float tilingFactor,
                                   int objectID)
                                 ...
                                 glm::vec4 tintColor)
{
    ...
    m_QuadBatch->submit(
        transform, glm::vec4(1.0f), QuadBatch::DefaultTexCoords, textureIndex, tilingFactor, -1);
    m_Stats.quadCount++;
}
...
void Renderer2D::drawCircle(
    const glm::mat4& transform, const glm::vec4& color, float thickness, float fade, int objectID)
{
    FM_PROFILE_FUNCTION();
    m_CircleBatch->submit(
        transform, color, thickness, fade, objectID, m_QuadBatch->getQuadVertexPositions());
    m_Stats.circleCount++;
}
...
void Renderer2D::quadPass()
{
    ...
    RenderPass pass;
    pass.Name = "QuadPass";
    pass.Execute = [self](RendererAPI& api) {
        self->m_QuadBatch->uploadToGPU();
        self->m_QuadBatch->bindTextures();
        self->m_QuadShader->bind();
        self->m_QuadPipeline->bind();
        api.drawIndexed(self->m_QuadBatch->getVertexArray(), self->m_QuadBatch->getIndexCount());
        self->m_Stats.drawCalls++;
    };
    m_PassQueue->addPass(pass);
}
...
void Renderer2D::quadInstancePass()
{
    ...
    pass.Execute = [self](RendererAPI& api) {
        ...
        api.drawIndexedInstanced(
            self->m_QuadBatch->getInstanceVertexArray(), 6, self->m_QuadBatch->getInstanceCount());
        self->m_Stats.drawCalls++;
    };
    m_PassQueue->addPass(pass);
}
...
void Renderer2D::circlePass()
{
    ...
    pass.Execute = [self](RendererAPI& api) {
        ...
        api.drawIndexed(self->m_CircleBatch->getVertexArray(),
...
void Renderer2D::linePass()
{
    ...
    pass.Execute = [self](RendererAPI& api) {
        ...
        api.setLineWidth(self->m_LineBatch->getLineWidth());
        api.drawLines(self->m_LineBatch->getVertexArray(), self->m_LineBatch->getVertexCount());
        self->m_Stats.drawCalls++;
    };
    m_PassQueue->addPass(pass);
}
...
void Renderer2D::textPass()
{
    ...
    pass.Execute = [self](RendererAPI& api) {
        ...
        api.drawIndexed(self->m_TextBatch->getVertexArray(), self->m_TextBatch->getIndexCount());
        self->m_Stats.drawCalls++;
    };
    m_PassQueue->addPass(pass);
}
